...
} elseif ($field['property'] === 'authorized_signature_image'  && !empty($value) ) {
...
if ($signedFile && !empty($signedFile['url'])) {
    ...
    if ($image) {
        $html .= '<td class="field-value"> <img src="data:' . htmlspecialchars($image['mime'], ENT_QUOTES, 'UTF-8') .';base64,' . $image['base64'] . '" style="max-height:150px; max-width:300px;"></td>';
    } else {
        $html .= '<td class="field-value"><div class="field-placeholder">Unable to load signature image. </div> </td>';
    }

} else {
    $html .= '<td class="field-value"> <div class="field-placeholder">Unable to access private signature image.</div></td>';
}
